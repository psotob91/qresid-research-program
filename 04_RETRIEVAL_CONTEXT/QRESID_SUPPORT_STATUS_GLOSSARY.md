...
| `ESTIMATOR_EQUIVALENT` | Stata and R fit the same model route. | Stata and R share compatible likelihood, parameterization, support, fitted-distribution parameters and tolerances for a route. | Use R as a coefficient/fitted-value benchmark before CDF/RQR layers. | Skipping CDF endpoint checks. | estimator equivalence audit, benchmark plans | benchmark mapping, source docs |
| `CDF_REPLAY_ONLY` | R validates residual math but does not refit the same estimator. | Stata estimates the model and exports enough parameters for R to recompute `F_low`, `F_high`, `U` and qres from cited formulas. | Validate CDF/PIT/RQR when estimator-level equivalence is unavailable. | Claiming R estimated the same route or coefficients. | estimator equivalence audit, evidence matrix | benchmark mapping |
| `APPROX_ESTIMATOR_BENCHMARK` | R estimates a nearby route. | R package parameterization is comparable but not pointwise identical to Stata. | Use as secondary evidence with clear limitations. | Promotion without analytic CDF replay or Stata validation. | estimator equivalence audit | benchmark mapping |
| `BENCHMARK_GATE_PASSED_IMPLEMENTATION_PENDING` | The model math and benchmark passed, but qresid does not implement it yet. | External estimator source/extraction/CDF/R benchmark are sufficient for a future implementation cycle, but `qresid.ado`, help, examples and certification have not been updated. | Plan implementation without re-opening source/CDF research. | Claiming current package support. | hurdle count gate, support matrix | registry, post-change sync |
| `PASS_EXTRACTION` | Required Stata postestimation values are extractable. | `e(b)`, equation names, `predict`, ancillary parameters and sample can be recovered robustly for benchmark purposes. | Move from source gate to endpoint benchmark. | Claiming residual implementation exists. | extraction audits | Stata model extraction rules |

...
| `QRESID_GGEFFECTS_WEB_STYLE_MASTER.md` | Estilo web docente derivado de `ggeffects`: navegacion, News/Reference, ritmo visual, salidas y graficos | Leer antes de cambios sustanciales en `docs/qresid-web/` |
| `QRESID_AI_LANGUAGE_SUPERVISOR_MASTER.md` | Limpieza de lenguaje publico: sin trazas internas, sobrepromesas ni tono artificial | Leer antes de editar help, README, web o textos docentes publicos |
| `QRESID_TEACHING_CONTENT_SUPERVISOR_MASTER.md` | Estructura minima de contenido docente: objetivo, modelo, salida Stata, grafico, interpretacion y contraste | Leer antes de crear o reescribir paginas docentes o estudios de caso |
| `QRESID_ADJUSTED_STUDENTIZED_RESIDUALS_MASTER.md` | Formulas y evidencia para `type(studentized)`, `type(adjusted)`, leverage, `prior.weights`, Scudilio-Pereira, topmodels y DHARMa | Leer antes de cambiar opciones de residuo ajustado/studentizado o claims de escala residual |

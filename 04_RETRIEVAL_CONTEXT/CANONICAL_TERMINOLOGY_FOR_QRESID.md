...
| R_EXACT_BENCHMARK | exact R benchmark, gold-standard R benchmark | R comparable si no es punto a punto | Mismo modelo, parametros, soporte, CDF, `U` y residuo comparables punto a punto contra R. | support glossary, benchmark mapping | Gold standard cuando existe equivalente R real. |
| R_APPROX_BENCHMARK | approximate R benchmark | exact benchmark | R tiene ruta comparable pero no identica en parametrizacion o soporte. | support glossary, benchmark mapping | Evidencia secundaria; requiere caveats. |
| STATA_INTERNAL_VALIDATION | Stata-only validation, internal validation | exact R benchmark | Validacion con comando oficial Stata, CDF, endpoints, PIT y checks internos cuando no hay equivalente R exacto. | support glossary, extraction rules | Debe indicar que no es benchmark exacto R-Stata. |
| STATA_EXTERNAL_ADO_VALIDATION | external ado validation | official Stata support | Validacion de un ado externo con fuente/version/licencia fijadas mas PMF/CDF y tests. | support glossary, source rules | No copiar codigo externo ni asumir licencia. |
| GATED_MODEL_FAMILY | model family gate | supported, experimental | Familia completa aun en investigacion por fuente, CDF, parametrizacion o benchmark pendiente. | support glossary, extension matrix | Usar para Hilbe/external counts no implementados. |
| NO_OFFICIAL_STATA_COMMAND | no official command | unsupported si hay ruta externa | Stata no trae estimador oficial; cualquier soporte requiere fuente externa o politica de estimacion separada. | support glossary, extension matrix | Activa research gate, no implementacion. |
| NO_ACTION_REQUIRED | no action needed | no sync needed si cambio soporte | La opcion/ruta no cambia la distribucion residual ni el claim actual. | support glossary, sync checklist | No omite sync si hay cambio real de soporte. |

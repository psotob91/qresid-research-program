...
`QRESID_EXTENSION_BRANCH_STATUS: COUNT_EXTENSION_PARTIAL_READY`
...
| grouped binomial `binreg, n()` | `READY_FOR_EXTENSION_PRERELEASE` | aliases `or`, `rr`, `rd` R-exact; `hr` Stata-internal |
| NB `nbreg, dispersion(mean)` | `READY_FOR_EXTENSION_PRERELEASE` | NB2-style route including tested no-offset, `offset()`, and `exposure()` |
...
- NB `dispersion(constant)`, `gnbreg`, `glm nbinomial`, and non-log links.

...
Status source: inverse Gaussian, expanded fweight, NB2 offset/exposure, grouped-binomial/binreg validation cycle plus Hilbe/count model coverage reconciliation, 2026-05-10. See `QRESID_IGAUSSIAN_FWEIGHT_EXTENSION_AUDIT.md` for IG/fweight logs, `QRESID_COUNT_MODEL_EXTENSION_EXECUTION_AUDIT.md` for NB/grouped-binomial logs, and `QRESID_HILBE_COUNT_MODEL_COVERAGE_PLAN.md` for count-model roadmap scope.
...
| Negative binomial | `nbreg, dispersion(mean)` | default/log | none; tested direct `fweight` separately | offset and exposure tested | `alpha`; `theta=1/alpha` | yes | PASS | `MASS::glm.nb`, `pnbinom` CDF check | PASS | claimed experimental | READY_FOR_EXTENSION_PRERELEASE | NB2 mean-dispersion route, including `offset()` and `exposure()`, is ready for local extension prerelease. | Keep `dispersion(constant)`, `gnbreg`, `glm nbinomial`, and untested NB variants gated. |
...
- Missing Tweedie, official zero-inflated/truncated/censored/specialized count
  ...
  `iweight`, NB variants beyond `dispersion(mean)`, and unsupported weighted routes do not invalidate the package

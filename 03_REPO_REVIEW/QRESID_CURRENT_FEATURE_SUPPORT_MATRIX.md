...
Status source: root `0c422cc`; `qresid` `32641db`; tag `qresid-v0.1.0-extension-prerelease.1`.
...
| Gamma | `glm, family(gamma)` | log/inverse/identity where estimable | none | not central | phi/shape/scale from GLM | yes | PASS | `glm(Gamma)`, `statmod` | PASS | claimed | READY_FOR_EXTENSION_PRERELEASE | You can use unweighted Gamma routes that passed benchmarks. | Public RC audit only. |
| fweight Gaussian/Poisson | `regress`; `poisson` with `[fweight=]` | model links above | `fweight` | Poisson log-link as tested | family-specific fitted distribution | yes | PASS | expansion/R checks for tested routes | PASS | claimed experimental | EXPERIMENTAL_VALIDATED_LOCAL | You can use direct fweights for tested Gaussian/Poisson routes, labelled experimental. | Do not generalize to all families. |
...
| inverse Gaussian | not implemented | future GLM links | none | not claimed | CDF/dispersion gate pending | no | N/A | `glm(inverse.gaussian)`, `statmod` | NOT_RUN | not_claimed | MISSING_NOT_BLOCKING | R has equivalents, but qresid does not claim this yet; package validity is not affected. | Open inverse Gaussian research gate. |
...
| NB weights | not implemented | NB only | gated | not claimed | weighted NB CDF semantics unresolved | no | gated | not established | NOT_RUN | not_claimed | MISSING_NOT_BLOCKING | Missing but not promised. | Research after unweighted NB public policy. |
| grouped-binomial weights | not implemented | grouped binomial | gated | not claimed | trials vs weights semantics unresolved | no | gated | not established | NOT_RUN | not_claimed | MISSING_NOT_BLOCKING | Missing but not promised. | Separate grouped-weight semantics gate. |
...
- Missing inverse Gaussian, Tweedie, ZIP/ZINB, hurdle, truncation, mixed models,
  `svy:`, `aweight`, `iweight`, NB weights and grouped-binomial weights do not
  invalidate the package because current public docs do not claim support for
  them.

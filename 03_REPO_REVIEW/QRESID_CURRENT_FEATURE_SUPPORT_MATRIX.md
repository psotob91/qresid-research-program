...
| Adjusted quantile residuals (studentized alias) | `type(adjusted)` canonical; `type(studentized)` exact alias on validated routes | READY_FOR_EXTENSION_PRERELEASE | THIS_REPORT | R_EXACT_BENCHMARK; R_CDF_REPLAY | `stats::glm`; `glmtoolbox::residuals2`; Scudilio-Pereira formula; manual R replay | `glmtoolbox::residuals2(type="quantile", standardized=TRUE)` for Gaussian; `qnorm(U)/sqrt(1-h)` replay for other tested GLM | one validated leverage adjustment `qres/sqrt(1-h)`; Gamma/IG additionally cite Scudilio-Pereira | no | no | Do not present adjusted/studentized/leverage-adjusted as separate methods when the formula is identical; weights and non-GLM routes use controlled error. |
| Gamma/inverse Gaussian dispersion override | `dispersion(#)` after `glm, family(gamma)` and `glm, family(igaussian)` | READY_FOR_EXTENSION_PRERELEASE | THIS_REPORT | R_CDF_REPLAY | `stats::glm`; Stata-exported fitted means; `statmod::qresiduals(..., dispersion=...)` convention | manual CDF replay equivalent to fixed-dispersion `statmod::qresiduals`; adjusted replay uses `qres/sqrt(1-h)` | user-fixed positive dispersion changes fitted CDF/PIT only; default uses Stata postestimation dispersion | no | no | Keep restricted to Gamma/IG GLM; no refit and no Tweedie/manual dispersion for other families. |

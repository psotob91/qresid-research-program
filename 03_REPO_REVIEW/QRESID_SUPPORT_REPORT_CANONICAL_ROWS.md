...
| Studentized quantile residuals | `type(studentized)` after unweighted `regress` and `glm` | READY_FOR_EXTENSION_PRERELEASE | THIS_REPORT | R_EXACT_BENCHMARK | `stats::glm`; `glmtoolbox::residuals2` | `glmtoolbox::residuals2(type="quantile", standardized=TRUE)` | leverage-standardized quantile residual `qres/sqrt(1-h)` | no | no | Keep restricted to unweighted `regress`/`glm`; other routes use controlled error. |
| Adjusted quantile residual variants | `type(adjusted)` reserved, controlled error | MISSING_NOT_BLOCKING | REPORT_SCOPE_ONLY | GATED_RESEARCH | candidate papers/packages; no validated implementation yet | none accepted | adjusted formula unresolved; Scudilio-Pereira formula requires paper/source audit before API activation | no | no | Keep gated until a formula and benchmark close. |

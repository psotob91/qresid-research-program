...
| Hurdle count Poisson/NB | pinned external `hplogit`; pinned external `hnblogit`; no `qresid.ado` implementation yet | MISSING_NOT_BLOCKING | SEPARATE_BENCHMARK | BENCHMARK_GATE_PASSED_IMPLEMENTATION_PENDING; STATA_EXTERNAL_ADO_VALIDATION; R_EXACT_BENCHMARK; R_CDF_REPLAY | `pscl::hurdle`; `glmmTMB`; `VGAM` candidates | `topmodels::qresiduals` candidate; manual hurdle CDF replay | hurdle discrete Dunn-Smyth with `uvar()` | no | no | Open separate qresid implementation cycle; dispatch cannot rely on `e(cmd)` because both commands leave `e(cmd)="ml"`. |

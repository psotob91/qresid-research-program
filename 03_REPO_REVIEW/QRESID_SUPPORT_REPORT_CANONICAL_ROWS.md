...
| Hurdle count Poisson/NB | no accepted Stata count-hurdle route yet | MISSING_NOT_BLOCKING | REPORT_SCOPE_ONLY | GATED_MODEL_FAMILY; NO_OFFICIAL_STATA_COMMAND | `pscl::hurdle`; `glmmTMB`; `VGAM` candidates | `topmodels::qresiduals` candidate; manual hurdle CDF replay after Stata route closes | hurdle discrete PIT/RQR formula documented | no | no | Pin/accept Stata estimator, then run three-dataset endpoint benchmarks. |

...
| QIC-006 | P1 | DO_NOT_IMPLEMENT_YET | NB | parametrizacion `alpha/theta/k`, NB1/NB2 y CDF no cerradas | mantener bloqueada o implementar stub controlado hasta investigacion externa | external research queue, count extraction rules | parametrization benchmark before activation | NB stable support | no |
...
| QIC-015 | P2 | DO_NOT_IMPLEMENT_YET | weights | semantica de pesos no cerrada | mantener fuera de soporte activo salvo casos con evidencia por familia | weights evidence review, benchmark mapping | family x weight-type tests after decision | weighted support | no |

## 3. Secuencia Recomendada

1. Crear tests minimos segun `QRESID_TEST_PLAN_BEFORE_CODE_CHANGES.md`.
...
## 4. Reglas De Entrada Para Cualquier Cambio

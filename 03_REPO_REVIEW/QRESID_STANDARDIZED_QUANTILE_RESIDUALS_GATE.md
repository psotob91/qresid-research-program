...
A different object, such as an adjusted, studentized, leverage-adjusted, bias-adjusted, or otherwise rescaled quantile residual, must not be added by analogy. It requires a route-specific mathematical formula, an R package/source-code audit when available, and a benchmark or replay test. Promoted routes are leverage-standardized quantile residuals after unweighted `regress` and tested `glm` families, exposed as `type(studentized)`, plus Scudilio-Pereira adjusted quantile residuals for unweighted Gamma and inverse Gaussian GLM, exposed as `type(adjusted)`.
...
| adjusted quantile residual | `r_i^{*qu}=r_i^{qu}/sqrt(1-h_ii)` | unweighted `glm gamma` and `glm igaussian` | Scudilio and Pereira (2020), Section 2.3; arXiv source pinned locally | no package function found; benchmark uses R CDF replay with exported `U` and `hatvalues(glm)` | arXiv PDF/TeX pinned; `glmtoolbox` formula inspected as related leverage-standardization source | Scudilio-Pereira explicitly studies continuous Gamma and inverse Gaussian GLMs. The adjustment rescales the quantile residual by the leverage factor; it does not define new CDF endpoints. | `READY_FOR_EXTENSION_PRERELEASE` |
...
- Public API is `type(quantile|studentized|adjusted)`. `type(quantile)` is default; `type(studentized)` is route-limited to unweighted `regress` and tested GLM families; `type(adjusted)` is route-limited to unweighted Gamma and inverse Gaussian GLM.

...
|---|---|---|---|---|---|---|---|
| RB-001 | resolved | RESOLVED_LOCAL_PRERELEASE | release certification | Current certification was development-only. | Pre-release local Stata certification gate added and run. | `certification/logs/10_May_2026_122105_certify_phase1.log`: `PASS_PRERELEASE_LOCAL_STATA_COMPONENTS`. | no |
...
| RB-005 | resolved | RESOLVED | benchmarks | Gaussian, Poisson and Bernoulli lacked R/Stata benchmark evidence. | Added and ran layered R/Stata benchmark for Gaussian, Poisson and Bernoulli. | `tests/logs/10_May_2026_122110_phase1_benchmark._r.log`: `QRESID_PHASE1_BENCHMARK_R_STATUS PASS`. | no |
...
| RB-018 | resolved | RESOLVED_LOCAL_PRERELEASE | GLM/link matrix | Phase 1 safe support lacked systematic GLM link, `binreg`, and offset/exposure matrix evidence. | Added individual `glm binomial`/`binreg` support, GLM/link Stata producer, R checker and HTML report. | `QRESID_GLM_LINK_MATRIX_AUDIT.md`; `10_May_2026_122110_glm_link_matrix_r.log`: `QRESID_GLM_LINK_MATRIX_R_STATUS PASS`. | no |
...
All prior `MUST_FIX_BEFORE_RC` blockers are closed for local prerelease readiness, including the GLM/link matrix requested after prerelease tagging. Remaining items are public-RC policy, future-family or post-prerelease hardening items.
